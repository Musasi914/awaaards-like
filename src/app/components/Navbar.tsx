...
export default function Navbar() {
    ...
    <div ref={copyContainersRef} className="menu__col">
      {MENU_ITEMS.map((item) => (
        <div key={item.id} className="menu__link">
          <a href={item.href} className="text-5xl md:text-6xl font-semibold">
            {item.label}
          </a>
        </div>
      ))}
    ...
    <div ref={copyContainersRef} className="menu__col">
      {TAG_ITEMS.map((item) => (
        <div key={item.id} className="menu__tag">
          <a href={item.href} className="text-xl md:text-2xl font-semibold">
            {item.label}
          </a>
        </div>
      ))}
  ...
  const HamburgerIcon = () => (
    <div
      ...
      className="menu__toggle-icon relative w-12 h-12 flex flex-col justify-center items-center border border-[var(--hamburger-icon-border)] rounded-full"
    >
      <span
        className={`absolute w-3.5 h-0.25 bg-foreground origin-center -translate-y-[3px] transition-all ${
          isMenuOpen ? "translate-y-[0] rotate-45 scale-110" : "-translate-y-[3px]"
        }`}
      />
      <span
        className={`absolute w-3.5 h-0.25 bg-foreground origin-center transition-all ${
          isMenuOpen ? "-translate-y-[0] -rotate-45 scale-110" : "translate-y-[3px]"
        }`}
      />
    </div>
  ...
  return (
    <header className="fixed w-full h-screen top-0 left-0 pointer-events-none overflow-hidden z-10">
      ...
      <div className="menu__bar fixed top-0 left-0 w-full p-4 flex justify-between items-center pointer-events-auto text-[var(--menu-fg-secondary)] z-10">
        ...
        <div className="menu__toggle-btn flex items-center gap-4 cursor-pointer">
          <div className="menu__toggle-label overflow-hidden">
            <p ref={menuToggleLabelRef} className="relative will-change-transform">
              Menu
            </p>
          </div>
          <HamburgerIcon />
      ...
      >
        <div
          ...
          className="menu__overlay-content fixed top-0 left-0 w-full h-screen overflow-hidden z-0 flex pointer-events-auto"
        >
          {/* メディアセクション */}
          <div ref={menuMediaWrapperRef} className="menu__media-wrapper flex-2 opacity-0 hidden md:block">
            <Image
              src="https://picsum.photos/seed/picsum/400/1000"
              width={400}
              height={1000}
              alt=""
          ...
          {/* コンテンツセクション */}
          <nav className="menu__content-wrapper flex-3 relative flex">
