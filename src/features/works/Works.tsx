...
export default function Works() {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // 画面左半分で、左端ならe.clientX、右端ならe.clientX - previewRef.current.offsetWidthになるように計算
    ...
    mouse.current.x =
      e.clientX - previewRef.current.offsetWidth * ratio + halfWidth;
    mouse.current.y = 100 + (e.clientY - window.innerHeight / 2) * 0.5;
  ...
  return (
    <section className="overflow-hidden">
      <div className="wrapper" ref={wrapperRef} aria-labelledby="works-title">
        ...
        <div className="relative umt-8" onMouseMove={handleMouseMove}>
          ...
          <div
            ref={previewRef}
            className="absolute top-0 left-0 z-50 overflow-hidden opacity-0 w-1/3 border-8 pointer-events-none hidden md:block"
          >
            {currentIndex != null && (
              <Image
